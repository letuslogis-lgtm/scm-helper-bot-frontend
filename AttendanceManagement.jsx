    ...
    return (
        <div className="p-6 bg-slate-100 min-h-[calc(100vh-64px)] slide-up flex flex-col gap-6 max-w-[1600px] mx-auto">
            <div className="flex justify-between items-center shrink-0">
                ...
                <button onClick={() => setIsUploadModalOpen(true)} className="bg-green-600 hover:bg-green-700 text-white text-sm font-bold px-5 py-2.5 rounded-lg shadow-sm transition-colors flex items-center gap-2">
                    ...
                    <div className="flex flex-wrap items-center w-full min-w-0 md:w-auto md:flex-1">
                        {activeTab === 'summary' ? (
                            <div className="flex gap-2 items-center">
                                ...
                                <input type="month" value={tempChartStartDate.substring(0, 7)} onChange={(e) => { setTempChartStartDate(e.target.value + '-01'); setChartStartDate(e.target.value + '-01'); }} className="border border-gray-200 rounded px-3 py-1.5 text-xs font-bold text-gray-700 outline-none focus:border-letusBlue" />
                                <span className="text-gray-400 mx-1">~</span>
                                <input type="month" value={tempChartEndDate.substring(0, 7)} onChange={(e) => {
                                    const [year, month] = e.target.value.split('-');
                                    const lastDay = new Date(year, month, 0).getDate();
                                    const newDate = `${e.target.value}-${lastDay}`;
                                    setTempChartEndDate(newDate); setChartEndDate(newDate);
                                }} className="border border-gray-200 rounded px-3 py-1.5 text-xs font-bold text-gray-700 outline-none focus:border-letusBlue" />
                            </div>
                        ) : (
                                            ...
                                            <table className="w-full text-left whitespace-nowrap">
                                                <thead className="bg-slate-50 border-b border-slate-200 text-xs text-slate-500 font-bold sticky top-0 z-10">
                                                    <tr>
                                                        ...
                                                        <th className="p-3 text-blue-600 text-center">실 투입 (생산성)</th>
